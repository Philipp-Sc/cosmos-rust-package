...
impl SupportedBlockchain {
    pub async fn channel(&self) -> anyhow::Result<Channel> {
        match &self.grpc_url {
            ...
            Some(grpc_url) => {
                get_channel(grpc_url.to_owned()).await
            }
        }
    }
}
...
pub async fn select_channel_from_grpc_endpoints(grpc_urls: &Vec<String>) -> anyhow::Result<String> {
    ...
    for grpc_url in grpc_urls.iter().map(|x| x.to_owned()) {
        join_set.spawn(async move {
            check_grpc_url(grpc_url).await
        });
    }
    let mut channel: Result<String, anyhow::Error> = Err(anyhow::anyhow!("Error: No gRPC url passed the check!"));
...
pub fn get_supported_blockchains() -> HashMap<String, SupportedBlockchain> {
    ...
    supported_blockchains.insert(
        ...
        SupportedBlockchain {
            ...
            grpc_url: None,
            governance_proposals_link: "https://wallet.keplr.app/chains/cosmos-hub/proposals/".to_string(),
        },
    );
...
pub async fn get_supported_blockchains_from_chain_registry(path: String, git_pull: bool, chain_registry_refresh_rate: Option<u64>) -> HashMap<String, SupportedBlockchain> {
    if git_pull {
        ...
        if let Some(refresh_rate) = chain_registry_refresh_rate {
            ...
            match (date_git_fetch_head,date_system) {
                (Ok(date_git_fetch_head),Ok(date_system)) => {
                    let d1: u64 = String::from_utf8_lossy(&date_git_fetch_head.stdout).to_string().replace("\n","").parse().unwrap_or(0);
                    let d2: u64 = String::from_utf8_lossy(&date_system.stdout).to_string().replace("\n","").parse().unwrap_or(0);
        ...
        if update {
            run_cmd("git", Some(vec!["-C", path.as_str(), "pull"]));
        }
    }


    let mut supported_blockchains: HashMap<String, SupportedBlockchain> = get_supported_blockchains();
    ...
    for (k, v) in supported_blockchains.iter_mut() {
        ...
        let mut try_these_grpc_urls: Vec<String> = chain_info.apis.grpc.iter().map(|x| format!("http://{}",x.address)).collect();
